...
<html><body><pre style="font: monospace">" > ${logfile}
...
if [ $SAS_count -gt 0 ]; then
  (
    ...
    echo ""
    echo "+------+------------------------+----+------+-----+------+------+------+------+------+------+"
    echo "|Device|Serial                  |Temp| Power|Start|Load  |Defect|Uncorr|Uncorr|Uncorr|Non   |"
    echo "|      |Number                  |    | On   |Stop |Unload|List  |Read  |Write |Verify|Medium|"
    echo "|      |                        |    | Hours|Count|Count |Elems |Errors|Errors|Errors|Errors|"
    echo "+------+------------------------+----+------+-----+------+------+------+------+------+------+"
  ) >> "$logfile"
  ...
  for drive in $SAS_list; do
    ...
    /hours:minutes/{split($6,a,":");sub(/h/,"",a[1]);AccPowerOnTime=a[1];} \
    ...
    END {
      ...
      device=device " " critSymbol;
        ...
        device=device " " warnSymbol;
      printf "|%-6s|%-24s| %3s|%6s|%5s|%6s|%6s|%6s|%6s|%6s|%6s|\n",
      device, serial, temp, AccPowerOnTime, startStop, loadUnload, defectList, \
